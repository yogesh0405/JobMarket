      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
        ...
        <div className="support-main-grid">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            ...
            {/* My Support Tickets Section (Left Column) */}
            {currentUser && (
              <div className="card support-card" style={{ padding: '20px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                  <h3 style={{ fontSize: '16px', fontWeight: 800, margin: 0, color: '#0f172a' }}>My Support Tickets</h3>
                  <button 
                ...
                {tickets.length > 0 ? (
                        ...
                        <tbody>
                          {tickets.map(t => (
                            <tr key={t.id} style={{ borderBottom: '1px solid var(--border)' }}>
                              <td style={{ padding: '10px 6px', fontWeight: '700', color: 'var(--text-primary)', whiteSpace: 'nowrap' }}>{t.ticket_number}</td>
                              <td style={{ padding: '10px 6px', color: 'var(--text-primary)', maxWidth: '160px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{t.subject}</td>
                              <td style={{ padding: '10px 6px' }}>
                                <span className={`status-badge status-${t.status}`} style={{ fontSize: '10.5px', padding: '3px 7px', borderRadius: '4px', textTransform: 'capitalize' }}>
                    ...
                    <div className="mobile-cards-view">
                      {tickets.map(t => (
                        <div key={t.id} className="mobile-ticket-card">
                          ...
                          <div className="mobile-ticket-actions" style={{ justifyContent: 'flex-end', marginTop: '8px' }}>
                            <button
                              className="btn btn-secondary btn-sm"
                              onClick={() => handleViewTicketDetails(t)}
                  ...
                  <div style={{ textAlign: 'center', padding: '24px 12px', color: 'var(--text-secondary)', fontSize: '13px' }}>
                    No support tickets submitted yet. Submit a query using the form.
                  </div>
                )}
              </div>
            )}
