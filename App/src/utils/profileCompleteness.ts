...
export function calculateCandidateProfileCompletion(user: any): SectionCompletionStatus {
  ...
  const hasAvatar = !!(user?.avatar_url || user?.avatarUrl || user?.profile_picture_url || user?.profilePictureUrl || user?.avatar);
  ...
  const hasTrade = !!(user?.tradeSpecialization || user?.trade_specialization || user?.headline || user?.trade);
  const hasExp = Array.isArray(user?.experience) 
    ? user.experience.length > 0 
    : !!(user?.experience || user?.total_experience || user?.totalExperience);
  const hasShift = !!(user?.preferredShift || user?.preferred_shift || user?.shift || user?.preferredShifts || true); // Shift default applies
  ...
  const hasLocation = !!(user?.location || user?.city || user?.address || user?.midcZone || user?.locality);
  ...
  const skillsCount = Array.isArray(user?.skills) 
    ? user.skills.length 
    : (typeof user?.skills === 'string' && user.skills.trim().length > 0 ? user.skills.split(',').length : 0);
  ...
  if (skillsCount >= 3) {
    ...
    skillsScore = 15; // Any skills present counts as completed section
  } else {
    skillsMissing.push('Trade Skills');
  }

  // 5. Education & Qualifications (15%)
  const hasEducation = Array.isArray(user?.education) 
    ? user.education.length > 0 
    : !!(user?.education || user?.highest_qualification || user?.qualification || user?.degree);
  ...
  const hasBio = !!(user?.bio || user?.about || user?.summary || user?.headline);
  const hasResume = !!(
    user?.resume_url || 
    user?.resumeUrl || 
    (typeof user?.resume === 'string' && user.resume.trim().length > 0) ||
    (user?.resume && (user.resume.url || user.resume.name || user.resume.file)) ||
    (Array.isArray(user?.resumes) && user.resumes.length > 0)
  );
